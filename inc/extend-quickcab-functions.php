...
function codeable_larry_custom_user_profile_fields( $user ) {
    ...
    <table class="form-table">
       ...
       <!--<tr id="clcuC">
            <th><label for="school_name"  title="school name"><?php esc_html_e( 'Scuola per cui prenotare', 'hello-elementor' ); ?></label></th>
            <td>
        ...
        <!--<tr id="clcuE">
            ...
            <td>
                <input type="text" name="district_name" id="district_name" value="<?php echo esc_attr( get_the_author_meta( 'district_name', $user->ID ) ); ?>" class="regular-text" />
...
function extend_quickcab_woocommerce_email_order_meta_fields( $fields, $sent_to_admin, $order ) 
{
    ...
    $fields['comprehensive_school'] = array(
        ...
        //'value' => get_post_meta( $order->id, 'comprehensive_school', true ),
    );
...
function extend_quickcab_checkout_field( $checkout ) {
    ...
    echo '</div>'; */
    echo '<div id="extend_quickcab_school_type_checkout_field">';
    ...
    echo '</div>'; /*
    echo '<div id="extend_quickcab_disctrict_name_checkout_field">';
...
function extend_quickcab_checkout_field_process() {
    ...
    if ( ! $_POST['school_type'] )
        wc_add_notice( __( 'Please enter school type field.' ), 'error' );
}
/**
...
function extend_quickcab_checkout_field_display_admin_order_meta($order){
    ...
    echo '<p><strong>'.__('Istituto comprensivo').':</strong> ' 
        . extend_quickcab_author_meta( 'comprehensive_school', get_customerorderid() ) . '</p>';
    /*echo '<p><strong>'.__('Scuola per cui prenotare').':</strong> ' 
        . extend_quickcab_author_meta( 'school_name', get_customerorderid() ) . '</p>';*/
    echo '<p><strong>'.__('Istituzione scolastica').':</strong> ' 
        . extend_quickcab_author_meta( 'school_type', get_customerorderid() ) . '</p>';
    echo '<p><strong>'.__('Cancellation').':</strong> ' 
        . esc_html($extquick_message_one) . '</p>';
    /*echo '<p><strong>'.__('District').':</strong> ' . extend_quickcab_author_meta('district_name', get_customerorderid()) . '</p>';*/
}
